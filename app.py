...
# Run this in a background daemon thread so the process doesn't block on the heavy DB read.
...
threading.Thread(target=_start_map_init_background, daemon=True).start()


# --- Start Background Threads (always run, even under gunicorn) ---
# Redis reload listener — reloads indexes when workers publish 'reload' after analysis
listener_thread = threading.Thread(target=listen_for_index_reloads, daemon=True)
listener_thread.start()

# Cron manager — checks and runs due cron jobs every 60s
...
# --- Flask dev server (only when running `python app.py` directly) ---
