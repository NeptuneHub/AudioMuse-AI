...
LYRICS_WHISPER_LANG_CONFIDENCE = float(os.environ.get(
    "LYRICS_WHISPER_LANG_CONFIDENCE", "0.7"))

WHISPER_MAX_NEW_TOKENS = int(os.environ.get(
    "LYRICS_WHISPER_MAX_NEW_TOKENS", "180"))

WHISPER_REPETITION_PENALTY = float(os.environ.get(
    "LYRICS_WHISPER_REPETITION_PENALTY", "1.15"))

WHISPER_NO_REPEAT_NGRAM = int(os.environ.get(
    "LYRICS_WHISPER_NO_REPEAT_NGRAM", "3"))

WHISPER_COMPRESSION_RATIO_THRESHOLD = float(os.environ.get(
    "LYRICS_WHISPER_COMPRESSION_RATIO", "2.4"))
...
_SUPPRESS_TOKEN_IDS: Tuple[int, ...] = tuple(range(LANGUAGE_TOKEN_START,
                                                   LANGUAGE_TOKEN_END + 1))
...
def _log_mel_spectrogram(wav: np.ndarray, mel_filters: np.ndarray) -> np.ndarray:
    ...
    if log_spec.shape[1] < WHISPER_FRAMES_PER_CHUNK:
        ...
        log_spec = np.pad(log_spec, ((0, 0), (0, pad_cols)),
                          mode='constant', constant_values=log_spec.min())
...
class _OnnxWhisperPipeline:
    def __init__(self, model_dir: str, intra_op_threads: int):
        ...
        intra_op_label = (
            'auto' if int(intra_op_threads) <= 0
        ...
        self.decoder_output_names: List[str] = [
            o.name for o in self.decoder_session.get_outputs()]

        sample_pkv = next(
            inp for inp in self.decoder_session.get_inputs()
            if inp.name.startswith('past_key_values.') and '.decoder.' in inp.name
        ...
        logger.info(
            ...
            self.num_heads, self.head_dim, len(self.lang_token_to_code),
        )
    ...
    def _empty_past_kv(self) -> Dict[str, np.ndarray]:
        ...
        for name in self.decoder_input_names:
            ...
            past_kv[name] = np.zeros(
                (1, self.num_heads, 1, self.head_dim), dtype=np.float32)
        return past_kv
    ...
    def _decode_step(self,
                     ...
                     past_kv: Dict[str, np.ndarray],
                     use_cache: bool) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
    ...
    def _detect_language(self, encoder_hidden_states: np.ndarray
                         ) -> Tuple[str, float]:
        input_ids = np.array([[SOT_TOKEN_ID]], dtype=np.int64)
        past_kv = self._empty_past_kv()
        logits, _ = self._decode_step(input_ids, encoder_hidden_states,
                                      past_kv, use_cache=False)
    ...
    def _decode_chunk(self,
                      ...
                      lang_token_id: int,
                      max_new_tokens: int,
                      beam_size: int) -> Tuple[str, float]:
        prompt_ids = [SOT_TOKEN_ID, lang_token_id, TRANSCRIBE_TOKEN_ID,
                      NO_TIMESTAMPS_TOKEN_ID]
        past_kv = self._empty_past_kv()

        input_ids = np.array([prompt_ids], dtype=np.int64)
        logits, named = self._decode_step(input_ids, encoder_hidden_states,
                                          past_kv, use_cache=False)
        ...
        if k == 1:
            ...
            for _ in range(max_new_tokens - 1):
                ...
                past_kv = self._absorb_present(past_kv, named,
                                               on_first_step=False)
                next_token, lp = self._sample_next(logits, generated)
                if next_token == EOT_TOKEN_ID:
                    break
            avg_logprob = (float(np.mean(token_logprobs))
                           if token_logprobs else float('-inf'))
        ...
        for _ in range(max_new_tokens - 1):
            ...
            for parent_idx, beam in enumerate(beams):
                if beam['finished']:
                    candidates.append(
                        (beam['log_prob_sum'], parent_idx, None, None))
                    continue
                last_tok = beam['tokens'][-1]
                input_ids = np.array([[last_tok]], dtype=np.int64)
                logits, named = self._decode_step(
                    input_ids, encoder_hidden_states, beam['past_kv'],
                    use_cache=True)
                ...
                for tok_id in top_idx:
                    ...
                    candidates.append(
                        (new_log_prob, parent_idx, tok_id, new_past_kv))
    ...
    def _absorb_present(self, past_kv: Dict[str, np.ndarray],
                        named_outputs: Dict[str, np.ndarray],
                        on_first_step: bool) -> Dict[str, np.ndarray]:
    ...
    def _compute_log_probs(self, logits: np.ndarray, generated: List[int]
                           ) -> np.ndarray:
        raw = logits[0, -1, :].copy()
        for tok in _SUPPRESS_TOKEN_IDS:
            if 0 <= tok < raw.shape[0]:
                raw[tok] = -1e30
        raw = _apply_repetition_penalty(raw, generated,
    ...
    def _sample_next(self, logits: np.ndarray, generated: List[int]
                     ) -> Tuple[int, float]:
    ...
    def transcribe(self, wav: np.ndarray,
                   ...
                   max_new_tokens: Optional[int] = None) -> Dict[str, object]:
        if wav is None or wav.size == 0:
            return {"text": "", "language": language or "",
                    "duration": 0.0, "avg_logprob": float('-inf')}
        ...
        intra_op_label = ('auto' if WHISPER_INTRA_OP_THREADS <= 0
                          else str(WHISPER_INTRA_OP_THREADS))
        logger.info(
            ...
            WHISPER_BEAM_SIZE, max_new_tokens, WHISPER_REPETITION_PENALTY,
            WHISPER_NO_REPEAT_NGRAM, WHISPER_COMPRESSION_RATIO_THRESHOLD,
            LYRICS_WHISPER_LANG_CONFIDENCE, intra_op_label,
            language, audio_duration,
        )
        t0 = time.time()

        n_chunks = max(1, int(np.ceil(len(wav) / WHISPER_CHUNK_SAMPLES)))
        windows = [wav[i * WHISPER_CHUNK_SAMPLES:
                       (i + 1) * WHISPER_CHUNK_SAMPLES]
        ...
        if language:
            ...
            logger.info(
                "Whisper-small: detected language=%r confidence=%.3f "
                "(threshold=%.2f)",
                detected_code, detected_prob, LYRICS_WHISPER_LANG_CONFIDENCE,
            )
            if detected_prob < LYRICS_WHISPER_LANG_CONFIDENCE:
                logger.info(
                    ...
                    detected_prob, LYRICS_WHISPER_LANG_CONFIDENCE,
                )
        ...
        for enc in encoder_outputs:
            ...
            if (cleaned and WHISPER_COMPRESSION_RATIO_THRESHOLD > 0
                ...
                logger.warning(
                    ...
                    WHISPER_COMPRESSION_RATIO_THRESHOLD, len(cleaned),
                )
                cleaned = ''
                avg_lp = float('-inf')
            if cleaned:
        ...
        avg_logprob = (float(np.mean(chunk_logprobs))
                       if chunk_logprobs else float('-inf'))
        logger.info(
            ...
            len(encoder_outputs), WHISPER_BEAM_SIZE, detected_code, avg_logprob,
        )
        return {
            "text": full_text,
...
def load_whisper_model() -> _OnnxWhisperPipeline:
    ...
    with _load_lock:
        ...
        _pipeline = _OnnxWhisperPipeline(LYRICS_WHISPER_MODEL_DIR,
                                         intra_op_threads=WHISPER_INTRA_OP_THREADS)
...
def unload() -> bool:
    ...
    _pipeline_dir = None
    try:
        for attr in ('encoder_session', 'decoder_session', 'tokenizer',
                     'mel_filters', 'lang_token_to_code',
        ...
        try:
            ...
            logger.exception(
                "Error during ONNX memory pool reset on Whisper unload")
