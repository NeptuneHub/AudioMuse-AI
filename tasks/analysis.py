            ...
            for idx, item in enumerate(tracks, 1):
                ...
                try:
                    ...
                    if needs_musicnn:
                        ...
                        save_track_analysis_and_embedding(item['Id'], item['Name'], item.get('AlbumArtist', 'Unknown'), analysis['tempo'], analysis['key'], analysis['scale'], top_moods, embedding, energy=analysis['energy'], other_features=other_features, album=item.get('Album', None), album_artist=item.get('OriginalAlbumArtist', None))
            ...
            for idx, album in enumerate(all_albums):
                ...
                if existing_count >= len(tracks) and not needs_clap_analysis and not needs_mulan_analysis:
                    ...
                    for item in tracks:
                        ...
                        try:
                            with get_db() as conn, conn.cursor() as cur:
                                cur.execute("UPDATE score SET album = %s, album_artist = %s WHERE item_id = %s", (album.get('Name'), item.get('OriginalAlbumArtist'), track_id_str))
                                conn.commit()
                            logger.info(f"[MainAnalysisTask] Updated album/album_artist for track '{item['Name']}' to '{album.get('Name')}' (main task)")
                        except Exception as e:
