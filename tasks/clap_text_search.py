...
def load_clap_cache_from_db():
    ...
    try:
        ...
        cur.execute("""
            ...
            JOIN score s ON ce.track_id = s.track_id
            ORDER BY ce.track_id
        """)
        ...
        for row in rows:
            ...
            if embedding.shape[0] != CLAP_EMBEDDING_DIMENSION:
                logger.warning(f"Skipping track_id {track_id}: wrong dimension {embedding.shape[0]} (expected {CLAP_EMBEDDING_DIMENSION})")
                continue
        ...
        _CLAP_CACHE['track_ids'] = track_ids_list
