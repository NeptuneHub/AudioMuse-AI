...
| `LYRICS_TEXT_MAX_COMPRESSION_RATIO`         | Compression ratio (zlib) used to filter out text that is not real lyrics. Highly repetitive content compresses far more than real lyrics, so text above this ratio is dropped before language detection / translation. Set to `0` to disable the gate. | `15.0` |
| `LYRICS_ENABLE_TRANSLATION`                 | When `true`, lyrics in a language other than English are translated to English before being embedded. Set to `false` if you prefer non-English lyrics to be skipped (treated as instrumental) instead of translated. | `true` |
| `LYRICS_LANG_CONFIDENCE_MIN`                | How confident the language detector must be (range `0.0`–`1.0`) before lyrics from a music server or external API are trusted. Below this, the lyrics are skipped (treated as instrumental) to avoid a wrong-language guess producing a bad translation. Lower it if valid lyrics are being skipped; raise it to be stricter. | `0.70` |
| `LYRICS_CJK_SCRIPT_MIN_RATIO`               | Minimum fraction of letters that must be CJK script (Korean Hangul, Japanese kana, or Chinese Han) for lyrics to be routed to a CJK translator **regardless of what langdetect/Whisper report**. Code-mixed K-pop / J-pop is routinely misdetected as English by langdetect (Latin-script bias), leaving the CJK part untranslated in the embedding (issue #553). Unicode script presence is a far more reliable CJK signal than a statistical detector on mixed text. Lower it to translate songs with only a little CJK; set `0` to disable the override. | `0.10` |
