...
| `AI_NAMING_TITLE_PROMPT`                    | Instructions used by the `title` naming style. Only the instructions are editable; the song sample is always appended after them. Edited in Setup -> AI Prompt. | see `config.py`                        |
