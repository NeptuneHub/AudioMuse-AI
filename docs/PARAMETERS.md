...
| `IVF_LAZY_LOAD_RETRY_SECONDS`               | Only the Flask process loads the indexes at startup; a worker task that queries one loads it on first query. Minimum seconds between retries when that load finds no index in the database, so a library with no analysis yet is not re-queried on every call. | `60` |
...
| **Album Creation General**                   |                                                                                                                            |                 |
| `ALBUM_CREATION_TRACKS`                      | Number of tracks in a created album (the CD format measured on real albums). The album is built from a song or from a few words describing it. | `12`            |
| `ALBUM_CREATION_MUSICNN_SHARE`               | Share of MusiCNN in the space a created album is measured in; the rest is DCLAP, which knows an album's instruments and voices better. `1.0` builds the album on MusiCNN alone. | `0.5`           |
| `ALBUM_CREATION_COHESION`                    | Target mean pairwise cosine between the tracks of a created album, in that mixed space. Plain similar-song sets sit at 0.92. The default is calibrated: there an album of several artists holds its instruments and voices together as tightly as a real album does. Lower is more eclectic, and 0.80 already changes genre. | `0.86`          |
| `ALBUM_CREATION_LYRIC_SHARE`                 | Share of the candidates kept by what their lyrics are about, the ones nearest the seed. A preference, not a rule: it applies while enough candidates remain and never judges a track without lyrics. `1.0` turns it off. | `0.25`          |
| `ALBUM_OF_THE_WEEK_PLAYLIST_NAME`            | Name of the playlist the scheduled (cron) Album of the Week run creates on every media server. The run looks for this exact name and replaces it in place. | `Album of the Week by AudioMuse-AI` |
...
| `AI_NAMING_PROMPT_MODE`                     | Clustering playlist naming style: `concept` (the AI picks one word and AudioMuse-AI adds the genre) or `title` (the AI writes the whole title from a sample of the playlist songs). Edited in Setup -> AI Prompt. | `concept`                              |
| `AI_NAMING_TITLE_PROMPT`                    | Instructions used by the `title` naming style. Only the instructions are editable; the song sample and the last titles already used are always appended after them. Edited in Setup -> AI Prompt. | see `config.py`                        |
...
| **Search by Recording (Neural Fingerprint)** |                                                                                           |                                        |
| `NEURAL_FINGERPRINT_ENABLED`                | Off by default. If false, the neural fingerprint stage of the analysis and its index build are skipped and Search by Recording leaves the menu, like `CLAP_ENABLED` and `LYRICS_ENABLED` do for their pages; turn it on from the Machine Learning Models switches of the setup wizard. Like every parameter here it is written to `app_config` on the first start that lacks it and read from there afterwards; a library that already holds neural fingerprints stores it as on, so upgrades never flip a model. | `false`                                |
| **Hyperbolic Explorer**                     |                                                                                            |                                        |
