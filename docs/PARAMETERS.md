...
| `IVF_TRAIN_SAMPLE_MAX`                      | Hard cap on the in-RAM k-means training sample used at index build time; keeps build memory bounded on very large libraries. Requires an index rebuild after change. | `50000`         |
| `MULTISERVER_CATALOG_CACHE_MAX_TRACKS`      | Cap on the tracks kept in the per-run server-alignment catalogue cache (summed across all servers); a catalogue that would exceed it is refetched instead of cached, bounding sweep memory. | `400000`        |
