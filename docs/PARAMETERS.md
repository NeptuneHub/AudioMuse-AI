...
| `LYRICS_QWEN_ASR_MODEL_DIR`                 | Path to the Qwen3-ASR-0.6B ONNX CPU build directory (replaces the previous Whisper integration). Must contain `onnx_models/encoder_conv.onnx`, `onnx_models/encoder_transformer.onnx`, `onnx_models/decoder_init.int8.onnx`, `onnx_models/decoder_step.int8.onnx`, `onnx_models/embed_tokens.bin`, and `tokenizer.json` at the root. Download with `huggingface-cli download Daumee/Qwen3-ASR-0.6B-ONNX-CPU --local-dir <dir>`. Pre-bundled in the official Docker image. | `/app/model/Qwen3-ASR-0.6B-ONNX-CPU` |
| `LYRICS_ASR_MIN_FREE_RAM_GB`                | Minimum free RAM (GB) each worker must see before loading its own Qwen3-ASR pipeline. If less is available the loader refuses; the song is treated as instrumental and the worker continues. The model needs ~5.7 GB resident plus activation headroom — keep this above ~6 GB. With multiple workers spinning up at once, late workers see less free RAM and naturally throttle (no cluster lock needed). Lower only if you're sure your node has spare RAM and the safety check is being too conservative. | `6` |
