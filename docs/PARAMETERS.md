...
| `HYPERBOLIC_INDEX_BANDS`                    | Number of radial bands the disk-paged Poincare index splits the projected catalogue into. More bands prune more aggressively during the exact branch-and-bound search. | `24` |
| `HYPERBOLIC_INDEX_CACHE_MB`                 | Process-wide memory cap (MB) for decoded Poincare index band vectors. The band directory stays in memory; band vectors are decoded on demand and evicted past this cap. | `256` |
