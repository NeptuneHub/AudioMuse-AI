...
| `IVF_DISK_CACHE_DIR`                        | Local directory for the mmap cell files. Defaults to the native-build data dir, else `/app/ivf_cache` in containers, else a temp dir. | `/app/ivf_cache` |
...
| `PATH_CANDIDATES_PER_STEP`                  | Number of candidate songs to retrieve from the IVF index for each step in the path                                              | `25`            |
