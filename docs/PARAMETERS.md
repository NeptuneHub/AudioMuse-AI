...
| `CLUSTERING_STALL_TIMEOUT_MINUTES`          | Last-resort safety valve: minutes during which NOTHING anywhere in a clustering run changed (no batch finished, failed, launched or disappeared, and no running batch advanced even one iteration) before the parent stops waiting and finishes with the best result it already has. The window restarts at every sign of life, so it is not a per-batch budget: a merely slow batch, or one a fresh worker picked up after the previous worker died, never trips it. `0` = never give up. | `60` |
