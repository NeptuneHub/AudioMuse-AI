...
| `VAD_VOICE_RECOGNITION`                     | Minimum seconds of voiced audio Silero VAD must detect before a track is sent to the selected ASR engine (`whisper_small` or `qwen_asr_0.6`, see `LYRICS_ASR_SELECT`) for lyric transcription. Tracks below this threshold are treated as instrumental/ambient and skip ASR entirely (the instrumental embedding sentinel is used instead). Use this knob to fine-tune instrumental/ambient song recognition in the lyrics analysis pipeline. Setting it very high (e.g. `1000`) effectively disables ASR transcription for every track, since no song can reach that much voiced audio within the 4-minute analysis clip. | `25` |
| `LYRICS_ASR_SELECT`                         | Which ASR engine runs in the lyrics pipeline. Both bundles ship in the Docker image; selection is runtime-only (also exposed in the setup wizard). Accepted values: `whisper_small` (openai/whisper-small ONNX, ~570 MB, built-in language detection with 0.7 confidence gate), `qwen_asr_0.6` (Qwen3-ASR-0.6B INT8 ONNX, ~2.4 GB, newer multilingual model). | `whisper_small` |
| `LYRICS_ASR_BEAM_SIZE`                      | Beam search width — **shared by both ASR engines**. 1 = pure greedy (fastest, most error-prone), 2 = sweet spot (catches stuck-loop attractors at ~2× greedy cost), 5 = Whisper-upstream default (max quality, ~5× cost). Each extra beam adds one extra decoder.run per generated token plus its own KV cache (~30-80 MB at a full 30 s chunk). | `2` |
| `LYRICS_ASR_CHUNK_SEC`                      | Target seconds per silence-aware split when Qwen3-ASR transcribes a track. Bigger values reduce per-chunk overhead slightly and give the decoder more context per inference (occasionally helps quality on songs whose verses span the split boundary). They do **not** meaningfully speed up total CPU transcription — encoder cost scales linearly with sequence length, so half-as-many-chunks-of-twice-the-length does roughly the same total work. Try 45-60 only if you notice premature EOS truncations at 30 s boundaries. | `30` |
| `LYRICS_QWEN_ASR_MODEL_DIR`                 | Path to the Qwen3-ASR-0.6B ONNX CPU build directory (used when `LYRICS_ASR_SELECT=qwen_asr_0.6`). Must contain `onnx_models/encoder_conv.onnx`, `onnx_models/encoder_transformer.onnx`, `onnx_models/decoder_init.int8.onnx`, `onnx_models/decoder_step.int8.onnx`, `onnx_models/embed_tokens.bin`, and `tokenizer.json` at the root. Pre-bundled in the official Docker image from the project release tarball `lyrics_model_qwen3_asr.tar.gz`. | `/app/model/Qwen3-ASR-0.6B-ONNX-CPU` |
| `LYRICS_WHISPER_MODEL_DIR`                  | Path to the Whisper-small ONNX bundle directory (used when `LYRICS_ASR_SELECT=whisper_small`). Must contain `encoder_model.onnx`, `decoder_model_merged.onnx`, `tokenizer.json` and the rest of the HuggingFace optimum export. Pre-bundled in the official Docker image from `lyrics_model_whisper.tar.gz`. | `/app/model/whisper-small-onnx` |
...
| `LYRICS_ASR_MIN_FREE_RAM_GB`                | Minimum free RAM (GB) each worker must see before loading its own Qwen3-ASR pipeline. If less is available the loader refuses; the song is treated as instrumental and the worker continues. The INT8 build has ~3 GB peak RSS plus ~500 MB-1 GB activation overhead and a transient doubling during load, so 5 GB leaves real headroom — important when 2-3 workers race the gate at once. Lower toward 4 only if you have a single worker. | `5` |
