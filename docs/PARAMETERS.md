...
| `CHROMAPRINT_BACKFILL_DISTRIBUTED`          | Off = the backfill runs serially on the one worker that finished the analysis phase (today's behavior). On = its targets are chunked and enqueued as `chromaprint_backfill` jobs that any idle worker drains, mirroring album analysis, so remote workers help instead of sitting idle. Per-track work is identical either way. | `false`         |
| `CHROMAPRINT_BACKFILL_BATCH_SIZE`           | Tracks per distributed backfill job (only used when `CHROMAPRINT_BACKFILL_DISTRIBUTED` is on). Larger = fewer queue rows but coarser progress/cancellation; smaller = work spreads more evenly across workers. Floored at 1.        | `50`            |
