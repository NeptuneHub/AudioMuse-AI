...
| `CLUSTERING_STALL_TIMEOUT_MINUTES`          | Last-resort safety valve: minutes during which NOTHING anywhere in a clustering run changed (no batch finished, failed, launched or disappeared) before the parent stops waiting and finishes with the best result it already has. It is not a per-batch budget, so a merely slow batch never trips it. `0` = never give up. | `240` |
...
| **Hyperbolic Explorer**                     |                                                                                            |                                        |
| `HYPERBOLIC_DEFAULT_LIMIT`                  | Default number of tracks returned by a Hyperbolic Explorer query.                         | `20`                                   |
| `HYPERBOLIC_MAX_LIMIT`                      | Hard cap on the number of tracks a single Hyperbolic Explorer query may return.           | `100`                                  |
...
| `HYPERBOLIC_CANDIDATE_OVERFETCH`            | Raw-space IVF candidate over-fetch multiplier before the hyperbolic re-ranking.            | `4`                                    |
...
| `HYPERBOLIC_JOURNEY_DEFAULT_LENGTH`         | Default number of tracks in a Geodesic Journey, both endpoints included. Steps are evenly spaced in hyperbolic arc length, so each one covers the same musical distance. | `14`     |
| `HYPERBOLIC_JOURNEY_MAX_LENGTH`             | Hard cap on the number of tracks a single Geodesic Journey may return.                     | `50`                                   |
| `HYPERBOLIC_JOURNEY_CANDIDATES_PER_STEP`    | IVF neighbors fetched per waypoint before the exact Poincare re-ranking picks one real track per step. The whole walk is one batched multi-probe, so this bounds the candidate pool regardless of catalogue size. | `60` |
| `HYPERBOLIC_JOURNEY_ANCESTRY_DIVE`          | How much deeper than the true geodesic the walk dips toward the origin, through a bump that is zero at both endpoints. `0` is the exact shortest geodesic (which already bows toward the shared root); higher values buy a longer detour through more general territory. | `0` |
...
| `HYPERBOLIC_JOURNEY_DEPTH_BOOST`             | The IVF index ranks by direction only, and tracks near the centre of the ball are rare, so a uniform probe returns almost nothing deep enough for the middle of a journey. The deepest waypoint tier probes `per-step * (1 + this)` neighbors instead of `per-step`. The probe count is unchanged; only `k` grows where the data is thin. | `3` |
| `HYPERBOLIC_JOURNEY_DEPTH_TIERS`             | How many depth buckets the waypoints are grouped into for that boost. Also caps the number of index calls per walk. | `3` |
| `HYPERBOLIC_JOURNEY_MAX_CANDIDATES`          | Hard ceiling on a whole walk's candidate pool, so the depth boost can never turn one request into an unbounded fetch on a large catalogue. | `4000` |
| **Plugin System**                           |                                                                                            |                                        |
