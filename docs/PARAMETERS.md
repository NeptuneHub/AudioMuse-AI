...
| `VAD_VOICE_RECOGNITION`                     | Minimum seconds of voiced audio Silero VAD must detect before a track is sent to Whisper for lyric transcription. Tracks below this threshold are treated as instrumental/ambient and skip Whisper entirely (the instrumental embedding sentinel is used instead). Use this knob to fine-tune instrumental/ambient song recognition in the lyrics analysis pipeline. Setting it very high (e.g. `1000`) effectively disables Whisper transcription for every track, since no song can reach that much voiced audio within the 4-minute analysis clip. | `25` |
