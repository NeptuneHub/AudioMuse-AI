...
| 1101 | Music Server Connection Error | Setup "Test connection" can't reach the server / returns nothing; also network failures classified as `HTTPError` / `MaxRetryError` / `RetryError` / `SSLError` / `RequestException` / `LyrionAPIError`; waveform track download fails | [app_setup.py](../app_setup.py), [app_waveform.py](../app_waveform.py), [error/error_manager.py](../error/error_manager.py) | Check the server URL is correct and reachable from the container; for a TLS failure confirm the certificate; confirm the server is running and the network/DNS path is open. |
...
| 2002 | Analysis Error | A per-album analysis task fails | [tasks/analysis.py](../tasks/analysis.py) album except | One album failed; check the log for the album/track and re-run analysis. The parent run now reports `failed_albums` and a sample of the child errors in its final details. |
