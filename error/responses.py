...
is built here from a registry code, so the body always carries ``error_code``,
``error_class`` and ``error_message`` next to the legacy ``error`` text the
pages already read, and the HTTP status always comes from the code unless the
route states its own. ``error_manager`` stays free of Flask so the worker can
import it; this module is the Flask-facing half, and it imports the rest of the
package inside its functions because every blueprint imports it.

Main Features:
* ``json_error`` answers with a code, an optional detail (the ``error`` alias,
  folded to one line and capped like the message, so a tool's stderr never
  reaches the page whole) and any extra keys the page expects.
* ``json_exception`` classifies a caught exception first, so a database,
  ...
  instead of the route's generic ones; one that stays on the route's code
  answers exactly like ``json_error``. It never puts the exception text in the
  body - the caller gets the registry message, the container log gets
  everything - and logs it exactly once with the request method and path:
  skipped when the route already logged it (``logger.exception``, which
  ``app_logging.LogSanitizingFilter`` marks), otherwise warning below 500 and
  error above. A route catching ``ValueError`` answers ``json_exception(exc,
  <code whose message explains the rejection>)``, never ``json_error``.
* A Werkzeug HTTP error a route's own ``except Exception`` caught (the 415 or
  400 ``request.get_json()`` raises) keeps its status and request code instead
  of turning into the route's 500, and still carries its extra keys.
...
* ``json_http_exception`` turns a Werkzeug HTTP error (a routing 404, a 405, a
  413 from the size cap) into the same JSON body on API paths while keeping
  Werkzeug's own response, so its headers (Allow, WWW-Authenticate) survive; an
  error without a code answers 500, never a JSON error body with 200.
* ``JSON_ERROR_PATH_PREFIXES`` / ``wants_json_error`` name the paths whose
  callers read JSON, so the global handlers and the auth barrier agree.
"""
